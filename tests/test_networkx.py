...
async def test_merge_node():
    ...
    assert isinstance(node.metadata.description, list)
    assert len(node.metadata.description) == 2
    assert description1 in node.metadata.description
    assert description2 in node.metadata.description
...
async def test_merge_nodes():
    ...
    node = await gdb.query_node(node1.id)
    assert isinstance(node.metadata.description, list)
    assert len(node.metadata.description) >= 1
